...
import com.vladlen.dotaautochessguide.model.MyError
...
class RetrofitException(private val _message: String?,
    ...
    private var _errorData : MyError? = null
    ...
    fun getErrorData() : MyError?  = _errorData

    private fun deserializeServerError() {
        if (_response?.errorBody() != null) {
            try {
                _errorData =  getErrorBodyAs(MyError::class.java)
            } catch (e: IOException) {
                Log.d("Error deserialization", e.message)
            }
        }
    }
